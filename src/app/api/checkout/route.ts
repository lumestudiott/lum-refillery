...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeProvider === 'wipay') {
      ...
      const formattedTotal = priceDollars.toFixed(2);
      const cleanUserSuffix = userId.replace(/[^a-zA-Z0-9_-]/g, '').slice(-8);
      const orderId = `sub_${tierId}_${Date.now()}_${cleanUserSuffix}`.slice(0, 48);
      ...
      params.append('avs', '0');
      params.append('data', JSON.stringify({ userId, type: 'subscription', tierId, billingCycle, orderId }));
