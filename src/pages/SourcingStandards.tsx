...
const SourcingStandards: React.FC = () => {
  return (
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-xl text-stone-600 leading-relaxed">
              We hold ourselves to the highest standards when selecting suppliers and products. 
              Here's how we ensure quality from farm to table.
            </p>
          </motion.div>
        </div>
      </section>
            ...
            {[
              ...
              >
                <div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center mb-6">
                  <standard.icon className="w-6 h-6 text-emerald-600" />
                </div>
                <h3 className="text-xl font-semibold text-stone-900 mb-4">{standard.title}</h3>
                <ul className="space-y-3">
                  {standard.points.map((point) => (
                    <li key={point} className="flex items-start gap-3 text-stone-600">
      ...
      <section className="py-16 bg-stone-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-serif font-bold text-stone-900 mb-4">Supporting Local</h2>
            <p className="text-stone-600 max-w-2xl mx-auto">
              We prioritize partnerships with local farmers and producers in Trinidad & Tobago, 
              reducing our carbon footprint while supporting the local economy.
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Truck, stat: '80%', label: 'Locally Sourced Products' },
              { icon: Users, stat: '25+', label: 'Local Partner Farms' },
              { icon: Award, stat: '100%', label: 'Fair Trade Commitment' }
            ].map((item, index) => (
              <motion.div
                key={item.label}
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="bg-white rounded-2xl p-8 text-center border border-stone-200"
              >
                <item.icon className="w-8 h-8 text-emerald-600 mx-auto mb-4" />
                <div className="text-4xl font-bold text-stone-900 mb-2">{item.stat}</div>
...
export default SourcingStandards;