...
const questions: Question[] = [
  {
    ...
    options: [
      { label: '1-2 people', value: 'small', icon: <Users className="w-6 h-6" /> },
      { label: '3-4 people', value: 'medium', icon: <Users className="w-6 h-6" /> },
      { label: '5+ people', value: 'large', icon: <Users className="w-6 h-6" /> },
    ]
  },
  {
    id: 'children',
  ...
  },
  {
    ...
    options: [
      { label: 'A few times a week', value: 'occasional', icon: <Utensils className="w-6 h-6" /> },
      { label: 'Almost every day', value: 'regular', icon: <Utensils className="w-6 h-6" /> },
      { label: 'Every meal, every day', value: 'frequent', icon: <Utensils className="w-6 h-6" /> },
    ]
  },
  {
    id: 'dietary',
  ...
  },
  {
    ...
    options: [
      { label: 'Best value for money', value: 'value', icon: <Home className="w-6 h-6" /> },
      { label: 'Quality & variety', value: 'quality', icon: <Leaf className="w-6 h-6" /> },
      { label: 'Premium & specialty items', value: 'premium', icon: <Leaf className="w-6 h-6" /> },
    ]
  },
  {
    id: 'frequency',
    ...
    options: [
      { label: 'Every 2 weeks (fortnightly)', value: 'fortnightly' },
  ...
  {
    id: 'storage',
    question: 'How much pantry/storage space do you have?',
    options: [
...
const Quiz: React.FC = () => {
  ...
  const getRecommendation = () => {
    const { household, children, cooking, dietary, staples, preference, frequency, storage, budget, priority } = answers;
    
    // Logic to determine best tier
    let score = 0;
    
    // Household size (bigger = more food needed)
    if (household === 'small') score += 1;
    else if (household === 'medium') score += 2;
    else if (household === 'large') score += 4;
    
    // Children (more mouths to feed)
    if (children === 'none') score += 0;
    else if (children === 'young') score += 1;
    else if (children === 'teens') score += 2;
    else if (children === 'mixed') score += 2;
    
    // Cooking frequency
    if (cooking === 'occasional') score += 0;
    else if (cooking === 'regular') score += 1;
    else if (cooking === 'frequent') score += 2;
    
    // Dietary preferences (organic = premium)
    if (dietary === 'organic') score += 3;
    else if (dietary === 'health') score += 1;
    
    // Preference
    if (preference === 'value') score += 0;
    else if (preference === 'quality') score += 2;
    ...
    // Budget caps the recommendation
          ...
          <>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <h1 className="text-2xl md:text-3xl font-serif font-bold text-stone-900 mb-8 text-center">
                ...
                <div className="space-y-4">
                  {questions[currentStep].options.map((option) => (
                    <button
                      ...
                      className={`w-full p-6 rounded-2xl border-2 text-left transition-all hover:border-emerald-500 hover:bg-emerald-50 ${
            ...
            {currentStep > 0 && (
              <button
                ...
                className="mt-8 text-stone-500 hover:text-stone-900 transition-colors flex items-center gap-2"
              >
                <ArrowLeft className="w-4 h-4" />
                Previous question
              </button>
          ...
          >
            ...
            <div className="bg-white rounded-2xl border border-stone-200 p-8 mb-8">
              <div className="text-4xl font-bold text-emerald-600 mb-2">
                ${recommendation?.price.monthly.toFixed(2)}
              </div>
              <div className="text-stone-500 mb-6">per month</div>
              
              <ul className="text-left space-y-3 mb-6">
                {recommendation?.items.slice(0, 5).map((item) => (
                  <li key={item.id} className="flex items-center gap-3 text-stone-600">
                    <Check className="w-5 h-5 text-emerald-500" />
                    <span>{item.quantity} {item.unit} {item.name}</span>
                  </li>
                ))}
                {(recommendation?.items.length || 0) > 5 && (
                  <li className="text-stone-400 ml-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 bg-stone-100 hover:bg-stone-200 text-stone-900 px-8 py-4 rounded-full font-medium transition-colors"
              >
                Retake Quiz
              </button>
            </div>
